...
- **Global scope**: `/home/deck/claude/blizz/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/deck/claude/blizz/.claude/notes/`)
...
2. Glob `/home/deck/claude/blizz/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/deck/claude/blizz/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/deck/claude/blizz/.claude/notes/` — works in any directory
